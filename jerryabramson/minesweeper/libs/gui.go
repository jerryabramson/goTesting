...
func drawWholeBoard(b *board, h bool) {
	...
	fmt.Printf("\033[H\033[1mCurrent Board\n\033[0m")
	printLine(80)
	fmt.Println()
	fmt.Printf("\033[0m      ")
	for x := 0; x < b.width; x++ {
...
func __prompt(b *board, sc *bufio.Scanner, msg string) (string, bool) {
    fmt.Printf(PROMPT_CSI)
    var input string
    fmt.Printf(msg)
    fmt.Printf(CLEAR_EOL_CSI)
    err := sc.Scan()
    if err {
        input = sc.Text()
    } else {
		...
		fmt.Printf("\033[31mI/O Error: %v\033[K\033[m", errMsg)
		fmt.Printf(PROMPT_CSI)
		fmt.Printf(CLEAR_EOL_CSI)
	}
	return input, err
...
func prompt(b *board) (int, int, error, bool) {
	...
	input, err := __prompt(b, sc, "Please choose a spot to check for a mine [origin at 1,1] (y,x)[,?]:")
